...
const GOOGLE_CONFIG = config.get('app.googleClientID') ? {
    ...
    callbackURL: "/api/v3/auth/customers/google/callback", 
    scope: ['profile', 'email'],
} : null;

// Cấu hình Facebook (sử dụng sau)
const FACEBOOK_CONFIG = config.get('app.facebookClientID') ? {
    ...
    callbackURL: "/api/v3/auth/customers/facebook/callback",
...
// --- 4. Chiến lược Facebook OAuth (Sẽ dùng sau) ---
if (FACEBOOK_CONFIG) {
